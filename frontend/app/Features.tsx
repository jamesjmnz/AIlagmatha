    ...
    <section className="pt-20 pb-32 px-4 bg-black/3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 justify-center gap-6 max-w-7xl mx-auto">
        {informations.map((info, index) => {
          ...
          return (
            <Card key={info.title} className="gap-1 py-10 px-2 h-full">
              <CardHeader>
                <div className="w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center mb-4 mx-auto md:mx-0">
                  {IconComponent && <IconComponent className="w-6 h-6 text-primary" />}
                </div>
                <CardTitle className="text-2xl mb-2 text-center md:text-left">{info.title}</CardTitle>
              </CardHeader>
              <CardContent>
                <CardDescription className="text-lg text-center md:text-left">{info.description}</CardDescription>
              </CardContent>
            </Card>
          )
        })}
